<script>
  ...
  $: if (show) {
    setTimeout(() => {
      const input = document.querySelector('.site-name-input');
  ...
  <div class="modal-backdrop" on:click={closeModal} on:keydown={handleKeydown}>
    <div class="modal-content" on:click|stopPropagation>
      <div class="modal-header">
        <h3>Save Site for TRP Report</h3>
        <button class="close-button" on:click={closeModal}>×</button>
      </div>

      <div class="modal-body">
        <p class="modal-description">
          Enter a name for this site to save the analysis and create a TRP report.
        </p>
      ...
      <div class="modal-footer">
        ...
        <button class="btn-primary" on:click={saveSite} disabled={saving || !siteName.trim()}>
          {#if saving}
            Saving...
          {:else}
            Save Site
